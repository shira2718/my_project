...
Summary:
    このファイルでは、ハードウェア・インタフェースを構造化します。
    USBスタックファームウェアは、PIC18とPIC24などの別のUSBマイコン上で動作するように
    コンパイルすることができます。
    USB関連する特殊機能レジスタとビット名は、一般にデバイス・ファミリ間で非常に似ていますが、
    命名の小さな違いが存在する。
...
/******************************************************************************
 ...
 このファイルには、USBハードウェア構造化層へのインタフェースを定義します。
...
#define _BSTALL     0x04        // バッファストールを有効にする
#define _DTSEN      0x08        // データトグル同期を有効にする
#define _INCDIS     0x10        // Address increment disable
#define _KEN        0x20        // SIE keeps buff descriptors enable
#define _DAT0       0x00        // 次はDATA0パケットが予想される
#define _DAT1       0x40        // 次はDATA1パケットが予想される
#define _DTSMASK    0x40        // DTS のマスクデータ
#define _USIE       0x80        // SIEがバッファを所有
#define _UCPU       0x00        // CPUがバッファを所有
...
#define CTRL_TRF_SETUP_ADDR_TAG 0x230
#define CTRL_TRF_DATA_ADDR_TAG  0x238
...
// バッファ ディスクリプタテーブル(BDT)のステータスレジスタ
typedef union _BD_STAT
{
    BYTE Val;
    struct{
        // CPUがバッファを所有している場合、これらの値である
        unsigned BC8:1;         // 転送データのサイズ
        unsigned BC9:1;         // (上位 8/9ビット)
        unsigned BSTALL:1;      // バッファストール有無のビット
        unsigned DTSEN:1;       // データトグル同期有無のビット
        unsigned INCDIS:1;      // 
        unsigned KEN:1;         // 
        unsigned DTS:1;         // データトグル同期値(DATA0/DATA1)
        unsigned UOWN:1;        // USB オーナービット(CPU=0/SIE=1)
    };
    struct{
        //USBモジュールがバッファを所有している場合、これらの値である
        unsigned :2;
        unsigned PID0:1;        // 最後の転送で受信した
        unsigned PID1:1;        // トークンPID の値
        unsigned PID2:1;        // (IN、OUT、SETUP トランザクションのみ)
        unsigned PID3:1;        // 
        unsigned :1;
    };
    struct{
        unsigned :2;
        unsigned PID:4;         // パケットPID
        unsigned :2;
    };
} BD_STAT;                      //Buffer Descriptor Status Register

// バッファ ディスクリプタテーブル(BDT) エントリーレイアウト
typedef union __BDT
{
    struct
    {
        BD_STAT STAT;             // バッファの設定と制御
        BYTE CNT;                 // 転送データのサイズ(下位 0～7ビット)
        BYTE ADRL;                // 転送データ格納先の下位アドレス
        BYTE ADRH;                // 転送データ格納先の上位アドレス
    };
    struct
    {
        unsigned :8;
        unsigned :8;
        WORD     ADR;             // 転送データ格納先のアドレス
    };
    DWORD Val;
    BYTE v[4];
} BDT_ENTRY;

// USB ステータスレジスタ(USTAT) レイアウト
typedef union __USTAT
{
    struct
    {
        unsigned char filler1:1;
        unsigned char ping_pong:1;         // 最後のトランザクションは奇数(1)か偶数(0)
        unsigned char direction:1;         // 最後のトランザクション方向(1=IN 0=OUT/SETUP)
        unsigned char endpoint_number:4;   // 最後に転送のあったエンドポイントの番号(0-7)
    };
    BYTE Val;
} USTAT_FIELDS;
