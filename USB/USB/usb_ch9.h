...
Summary:
    このファイルには、データ構造、定数、およびUSB2.0仕様の第9章で説明する
    USBデバイスフレームワークプロトコルをサポートするために使用されているマクロを
    定義しています。
...
// ******************************************************************
// Section: USB Device Qualifier ディスクリプタの構造体
// ******************************************************************
// この構造体はデバイス修飾子の記述子を記述します。
// デバイスが "その他"の速度をサポートしている場合、
// デバイス修飾子の記述子は、全体的なデバイスの情報を提供します。
//
// Note: 高速デバイスは、"他の"速度（フルすなわちまたは低をサポートする事ができる。
//       もしそうであれば、デバイス修飾子および他の高速記述子を実装する必要があるかもしれません。
...
// ******************************************************************
// Section: USBセットアップパケットの構造体
// ******************************************************************
// この構造は、USB標準デバイスリクエストの設定パケットに含まれるデータ(8バイト)を記述する。
// これは、デバイスを制御し、設定するデバイスへのホストから送信されるデータパケットである。
// note：セットアップパケットと標準デバイスリクエストの使用方法の詳細は、
//       USB 2.0仕様(第9章-2項)を参照してください。
...
typedef union __attribute__ ((packed))
{
    ...
    {
        BYTE bmRequestType;   // リクエストの特性情報
        BYTE bRequest;        // リクエストの種類
        WORD wValue;          // bRequestが決める値
        WORD wIndex;          // bRequestが決めるインデックスやオフセット値
        WORD wLength;         // データが付随する場合のデータサイズ
    };
    struct __attribute__ ((packed))
    {
        unsigned :8;
    ...
    struct __attribute__ ((packed))
    {
        unsigned Recipient:5;      // リクエストの受信者(0:デバイス 1:インターフェイス 2:エンドポイント 3:その他)
        unsigned RequestType:2;    // リクエストのタイプ(0:標準 1:クラス 2:ベンダ 3:予約)
        unsigned DataDir:1;        // データの転送方向(0:Host -> Device 1:Device -> Host)
        unsigned :8;
    ...
    struct __attribute__ ((packed))
    {
        ...
        {                               // ------   ------------------------
            ...
            struct
            {
                BYTE    recipient:  5;  // リクエストの受信者
                BYTE    type:       2;  // リクエストのタイプ
                BYTE    direction:  1;  // データの転送方向
            };
        }requestInfo;
    };
    struct __attribute__ ((packed))
